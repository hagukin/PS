...
#include <iostream>
/*
정석적인 풀이는 서브트리들로 나눠가며(DFS) 조건을 확인해나가는 풀이이지만,
나는 다른 풀이법으로 풀이했다.
...
코테에서 쓰기에는 그다지 좋은 방법도 아니고, 지금 생각해면
123 4 567일때
중간 4를 기준으로
123 456 두 서브트리로 나눈다음
두 서브트리의 중간점만 구해도 빠르게 자식노드를 구할 수 있는데
이걸 다른 방식으로 풀어버려 필요 이상으로 지나치게 오래 걸린 것 같다.
