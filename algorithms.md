...
[참고글 - 개요](https://velog.io/@kimdukbae/%EC%9E%90%EB%A3%8C%EA%B5%AC%EC%A1%B0-%EC%84%B8%EA%B7%B8%EB%A8%BC%ED%8A%B8-%ED%8A%B8%EB%A6%AC-Segment-Tree)  

세그먼트 트리에서 고려해줘야 할 부분은 다음과 같다.
1) 배열로부터 세그먼트 트리 생성
2) 세그먼트 트리에서 구간합 구하기
...
우선 세그먼트 트리 자체는 완전이진트리와 유사한 이진트리인데, 때문에 배열을 사용해 완전이진트리를 저장하듯이 정보를 저장한다.  
